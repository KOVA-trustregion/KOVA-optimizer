...
class KalmanOptimizer(optimizer.Optimizer):
    """Optimizer that implements the Kalman algorithm.
    """

    def __init__(self, var_list=[], learning_rate=1., eta=0.01, onv_coeff=1., batch_size=10,
               onv_type='batch_size', use_locking=False, name="Kalman"):
        ...
        self.onv_type = onv_type # ONV=Observation Noise Var. this is a string that indicate how to calculate self.observation_noise_var
    ...
    def apply_kalman(self, innovation=None, ratio=None, q_function_gradient_list=[], list_of_vars=[]):
        if self.onv_type == 'batch_size':
            self.observation_noise_var = self.onv_coeff * self.batch_size
        elif self.onv_type == "ratio":
            self.observation_noise_var = self.onv_coeff * (1. / (ratio + self.eps)) * self.batch_size
        elif self.onv_type == "max-ratio":
            self.observation_noise_var = self.onv_coeff * tf.maximum(1., (1. / (ratio + self.eps))) * self.batch_size
        elif self.onv_type == "empirical-variance":
            self.observation_noise_var = self.onv_coeff * tf.reduce_sum(tf.square(innovation))
            print ("self.observation_noise_var", self.observation_noise_var)
        elif self.onv_type == "empirical-variance-mean":
            self.observation_noise_var = self.onv_coeff * tf.reduce_mean(tf.square(innovation))

        self.t+=1
        tsuperstart = time.time()
        q_function_gradient = - self.concatenate_tensors(q_function_gradient_list)

        p_hat_predicted = tf.multiply(self.covariance, 1 / (1 - (self.eta)))
        P_theta_r = tf.matmul(p_hat_predicted, q_function_gradient)
        P_r = tf.add(tf.matmul(q_function_gradient, P_theta_r, transpose_a=True),
                     self.observation_noise_var * tf.eye(self.batch_size))

        self.K = tf.matmul(P_theta_r, tf.matrix_inverse(P_r))
        weight_update = tf.matmul(self.K, tf.expand_dims(innovation, 1))
        KP = tf.matmul(self.K, P_r)
        KPK = self._learning_rate * tf.matmul(KP, self.K, transpose_b=True)
        self.covariance = tf.subtract(p_hat_predicted, KPK)
    ...
    def convert_vector_to_variables(self, vector, list_of_var, method="tf"):
        ...
        var_list = []
        if method == "tf":
            for i, network_mat in enumerate(list_of_var):  #:TODO check this function
